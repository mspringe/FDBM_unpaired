...
from .sampling_lib.solvers import em
from .opt_lib.alpha_diffusion import resolve_acc_steps
import numpy as np
...
def plot_samples(samples, ys):
    f, axs = plt.subplots(1, len(samples), figsize=[len(samples)*2, 2])
    for img, y, ax in zip(samples, ys, axs.flatten()):
        print(img.shape)
...
def eval_sampling(apply_fn, params, pi_0, pi_1, solver_steps=100,
                  num_reps=10, batch_size=256, sqrt_eps=1.0):
    # num_samples
    N_0 = len(pi_0)
    N_1 = len(pi_1)
    # preparing outputs
    ...
    # sharding
    params, key = dist.map_sharding(dist.REPLICATE_SHARDING, params, key)
    ...
    for R in range(num_reps):
        ...
        while idx_1 < N_1:
            ...
            imgs_backwd = em(apply_fn, params, X_1, jax.device_put(_0[:end-idx_1], dist.DP_SHARDING),
                            num_steps=solver_steps,
                            key=subkey, eps=sqrt_eps**2)
        ...
        while idx_0 < N_0:
            ...
            imgs_forwd = em(apply_fn, params, X_0, jax.device_put(_1[:end-idx_0], dist.DP_SHARDING),
                            num_steps=solver_steps,
                            key=subkey, eps=sqrt_eps**2)
